...
export async function getNativePreparation(code: unknown) {
  ...
  return { ventaId: order.codigo_pedido, cliente: "", total: number(order.total), lineas: (details || []).map(detail => {
    ...
    return { ventaId: order.codigo_pedido, codigo: product?.codigo || "", producto: product?.nombre || "Producto", cantidadPedido: number(detail.cantidad_presentacion), presentacion: presentation?.nombre || "Unidad", factorPresentacion: factor, unidadesBase: number(detail.cantidad_unidades_base), precio: number(detail.precio_aplicado), subtotal: number(detail.subtotal), cantidadPreparada: number(prep?.cantidad_preparada) / factor, estadoLinea: prep?.estado || "PENDIENTE", cantidadFaltante: number(prep?.cantidad_faltante) / factor, motivo: prep?.motivo || "", observacion: prep?.observacion || "", estadoStock: reserve?.estado || "SIN RESERVA", stockFisico: number(currentStock?.stock_fisico), stockReservado: number(currentStock?.stock_reservado), stockEnRuta: number(currentStock?.stock_en_ruta), stockDisponible: number(currentStock?.stock_disponible), productoId: detail.producto_id };
  }) };
}
...
export async function saveNativePreparation(token: string, payload: PreparationPayload) {
  ...
  const lines = (payload.lineas || []).map(line => ({ producto_id: ids.get(String(line.codigo || "")) || line.productoId, cantidad_preparada: number(line.cantidadPreparada) * Math.max(1, number(line.factorPresentacion || 1)), motivo: String(line.motivo || ""), observacion: String(line.observacion || "") }));
