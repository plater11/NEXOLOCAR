...
export async function getNativePreparation(code: unknown) {
  ...
  return { ventaId: order.codigo_pedido, cliente: "", total: number(order.total), lineas: (details || []).map(detail => {
    ...
    const factor = Math.max(1, number(presentation?.factor || detail.factor_presentacion || 1));
    return { ventaId: order.codigo_pedido, codigo: product?.codigo || "", producto: product?.nombre || "Producto", cantidadPedido: number(detail.cantidad_presentacion), presentacion: presentation?.nombre || "Unidad", factorPresentacion: factor, unidadesBase: number(detail.cantidad_unidades_base), precio: number(detail.precio_aplicado), subtotal: number(detail.subtotal), cantidadPreparada: number(prep?.cantidad_preparada), estadoLinea: prep?.estado || "PENDIENTE", cantidadFaltante: number(prep?.cantidad_faltante), motivo: prep?.motivo || "", observacion: prep?.observacion || "", estadoStock: reserve?.estado || "SIN RESERVA", stockFisico: number(currentStock?.stock_fisico), stockReservado: number(currentStock?.stock_reservado), stockEnRuta: number(currentStock?.stock_en_ruta), stockDisponible: number(currentStock?.stock_disponible), productoId: detail.producto_id };
  }) };
}
...
export async function getNativeBulkStockTemplate() {
  ...
  return (presentations || []).map(row => {
    ...
    .sort((left, right) => compareProductCodes(String(left.codigo), String(right.codigo)) || String(left.presentacion).localeCompare(String(right.presentacion), "es"));
}
...
export async function correctNativeOrder(token:string,payload:EntityPayload){
  const order=await orderByCode(payload.ventaId),db=getSupabaseAdminClient(),clientName=String(payload.cliente||"").trim();
  const{data:clients,error:ce}=await db.from("clientes").select("id,nombre").ilike("nombre",clientName);if(ce)throw ce;
  const client=clients?.find((row:any)=>row.nombre.toLowerCase()===clientName.toLowerCase())||clients?.[0];if(!client)throw new Error("Cliente no encontrado.");
  const source=Array.isArray(payload.items)?payload.items as EntityPayload[]:[],codes=source.map(row=>String(row.codigo||"").toUpperCase());
  const{data:products,error:pe}=await db.from("productos").select("id,codigo,precio_venta").in("codigo",codes);if(pe)throw pe;
  const productIds=(products||[]).map((row:any)=>row.id),{data:presentations,error:pre}=productIds.length?await db.from("presentaciones").select("id,producto_id,factor,precio").in("producto_id",productIds).eq("activo",true):{data:[],error:null};if(pre)throw pre;
  const map=new Map<string,any>((products||[]).map((row:any)=>[row.codigo,row])),items=source.map(row=>{const product=map.get(String(row.codigo||"").toUpperCase());if(!product)throw new Error(`Producto ${row.codigo} no encontrado.`);const quantity=Math.max(0,number(row.cantidadPresentacion??row.cantidad)),factor=Math.max(1,number(row.factorPresentacion)||1),presentation=(presentations||[]).find((item:any)=>item.producto_id===product.id&&number(item.factor)===factor),price=number(row.precioPresentacion??row.precioVenta??presentation?.precio??product.precio_venta);return{producto_id:product.id,presentacion_id:presentation?.id||"",cantidad:quantity,factor,cantidad_base:number(row.cantidadUnidadesBase)||quantity*factor,precio:price,subtotal:quantity*price};});
