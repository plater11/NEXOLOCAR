...
export async function executeCompatRead(fn: string, args: unknown[]) {
  ...
  if (fn === "obtenerEmisiones") {
    ...
    return rows.map(row => ({ ventaId: row.codigo_pedido, fecha: row.fecha, cliente: row.cliente, telefono: row.telefono, total: Number(row.total), itemsCount: row.items.reduce((sum: number, item: { cantidad: number }) => sum + Number(item.cantidad), 0), lineas: row.items.length, observaciones: row.observaciones || "", estadoOperativo: row.estado_operativo, estadoEntrega: row.estado_entrega, estadoCobro: row.estado_cobranza, subestadoOperativo: row.subestado_operativo, motivoIncidencia: row.motivo_incidencia, fechaReprogramada: row.fecha_reprogramada, fechaVencimientoCobro: row.fecha_vencimiento_cobro, codigoImpresion: row.estado_boleta === "EMITIDA" ? "EMITIDA" : "", items: row.items }));
  }
