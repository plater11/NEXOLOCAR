...
function Orders({ orders, call, refresh, notify, onOrderUpdated }: {
    ...
    function openDelivery(result = "ENTREGADO") {
        ...
        setDelivery({ resultado: result, fechaEntrega: today(), medioPago: result === "ENTREGADO" && balance > .01 ? "CREDITO" : "SIN_COBRO", monto: 0, observacion: "", fechaPromesa: result === "ENTREGADO" && balance > .01 ? addDays(today(), 3) : "" });
    ...
    async function submitDelivery(e: FormEvent) {
        ...
        if (["RECHAZADO", "REPROGRAMADO"].includes(delivery.resultado) && !delivery.observacion.trim()) return notify("Indica el motivo de la incidencia.");
        if (delivery.resultado === "REPROGRAMADO" && !delivery.fechaPromesa) return notify("Indica la nueva fecha de entrega.");
        ...
        try {
            ...
            const message = await call<string>("guardarCobranzaPedido", [{ ...detail, solicitudId: crypto.randomUUID(), ventaId: detail.ventaId, cliente: detail.cliente, totalPedido: detail.total, estadoEntrega: delivery.resultado, fechaEntrega: delivery.fechaEntrega, estadoPago: totalPaid >= detail.total - .01 ? "TOTAL" : totalPaid > .01 ? "PARCIAL" : "PENDIENTE", efectivo: Number(detail.efectivo || 0) + efectivo, yape: Number(detail.yape || 0) + yape, plin: Number(detail.plin || 0) + plin, transferencia: Number(detail.transferencia || 0) + transferencia, pos: detail.pos || 0, otros: detail.otros || 0, fechaPromesa: delivery.resultado === "REPROGRAMADO" || (delivery.resultado === "ENTREGADO" && totalPaid < detail.total - .01) ? delivery.fechaPromesa : "", medioPrometido: totalPaid < detail.total - .01 ? "POR CONFIRMAR" : "", observacion: delivery.observacion, itemsEntregados: detail.items.map(item => ({ codigo: item.codigo, cantidadPedida: item.cantidad, cantidadEntregada: delivery.resultado === "ENTREGADO" ? item.cantidad : 0 })) }]);
    ...
    return <div className="orders-page orders-flow-v2">
        ...
        {deliveryOpen && detail && <div className="modal-bg delivery-action-bg"><form className="modal delivery-action-modal" onSubmit={submitDelivery}><button type="button" className="close" onClick={() => setDeliveryOpen(false)}>×</button><small className="eyebrow">ENTREGA Y COBRO</small><h2>{detail.ventaId}</h2><label>Resultado<select value={delivery.resultado} onChange={e => setDelivery({...delivery,resultado:e.target.value})}><option value="ENTREGADO">Entregado</option><option value="RECHAZADO">Rechazado</option><option value="REPROGRAMADO">Reprogramado</option></select></label><label>Fecha<input type="date" required value={delivery.fechaEntrega} onChange={e => setDelivery({...delivery,fechaEntrega:e.target.value})}/></label>{delivery.resultado === "ENTREGADO" && <><label>Condición de pago<select value={delivery.medioPago} onChange={e => setDelivery({...delivery,medioPago:e.target.value,monto:e.target.value === "CREDITO" ? 0 : Math.max(0,Number(detail.saldo ?? detail.total))})}><option value="CREDITO">Por cobrar</option><option value="EFECTIVO">Efectivo</option><option value="YAPE">Yape</option><option value="PLIN">Plin</option><option value="TRANSFERENCIA">Transferencia</option></select></label>{delivery.medioPago !== "CREDITO" && <label>Importe cobrado<input type="number" min="0.01" max={Math.max(0,Number(detail.saldo ?? detail.total))} step="0.01" value={delivery.monto || ""} onChange={e => setDelivery({...delivery,monto:Number(e.target.value)})}/></label>}</>}{(delivery.resultado === "REPROGRAMADO" || (delivery.resultado === "ENTREGADO" && delivery.medioPago === "CREDITO")) && <label>{delivery.resultado === "REPROGRAMADO" ? "Nueva fecha" : "Compromiso de pago"}<input type="date" required value={delivery.fechaPromesa} onChange={e => setDelivery({...delivery,fechaPromesa:e.target.value})}/></label>}<label>Comentario<textarea required={delivery.resultado !== "ENTREGADO"} value={delivery.observacion} onChange={e => setDelivery({...delivery,observacion:e.target.value})} placeholder="Motivo, referencia o indicación"/></label><button className="primary" disabled={deliveryBusy}>{deliveryBusy ? "REGISTRANDO…" : "CONFIRMAR OPERACIÓN"}</button></form></div>}
    </div>;
