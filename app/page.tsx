...
function Sales({ products, clients, call, refreshProducts, notify, online }: {
    ...
    function add(p: Product) {
        ...
        setCart(x => x.some(i => i.codigo === p.codigo) ? x.map(i => i.codigo === p.codigo ? (() => { const cantidad = i.cantidad + 1, unidadesSueltas = cantidad * i.factor; if (unidadesSueltas > p.stock) { notify(`Stock insuficiente. Disponible: ${p.stock} ${info.detalleUnidad}.`); return i; } return { ...i, cantidad, unidadesSueltas, fraccion: cantidad % 1 }; })() : i) : [...x, { codigo: p.codigo, nombre: p.nombre, cantidad: 1, precioVenta: info.precioPresentacion, unidad: p.unidad, factor: info.factor, unidadesSueltas: info.factor, detalleUnidad: info.detalleUnidad, stock: p.stock, nombrePresentacion: info.nombrePresentacion, fraccion: 0 }]);
    ...
    function changeUnits(item: SaleItem, delta: number) {
        setCart(rows => rows.flatMap(row => {
            if (row.codigo !== item.codigo)
                return [row];
            const cantidad = row.cantidad + delta;
            ...
            return [{ ...row, unidadesSueltas, cantidad, fraccion: cantidad % 1 }];
        }));
    }
    function addFraction(p: Product, fraction: number) {
        const info = presentation(p), baseUnits = info.factor * fraction;
        if (!info.fractions.some(option => option.value === fraction) || (!Number.isInteger(baseUnits) && String(p.controlaDecimales || "NO").toUpperCase() !== "SI")) return notify("Esta fracción no es válida para la presentación.");
        setCart(rows => { const current = rows.find(row => row.codigo === p.codigo), quantity = Number((current?.cantidad || 0) + fraction), units = quantity * info.factor; if (units > p.stock) { notify(`Stock insuficiente. Disponible: ${p.stock} ${info.detalleUnidad}.`); return rows; } return current ? rows.map(row => row.codigo === p.codigo ? { ...row, cantidad: quantity, unidadesSueltas: units, fraccion: quantity % 1 } : row) : [...rows, { codigo: p.codigo, nombre: p.nombre, cantidad: fraction, precioVenta: info.precioPresentacion, unidad: p.unidad, factor: info.factor, unidadesSueltas: baseUnits, detalleUnidad: info.detalleUnidad, stock: p.stock, nombrePresentacion: info.nombrePresentacion, fraccion: fraction }]; });
    }
    ...
    function setUnits(item: SaleItem, units: number) {
        const safeQuantity = Math.max(0, Number(units) || 0), safeUnits = safeQuantity * item.factor;
        ...
        setCart(rows => safeQuantity === 0 ? rows.filter(row => row.codigo !== item.codigo) : rows.map(row => row.codigo === item.codigo ? { ...row, unidadesSueltas: safeUnits, cantidad: safeQuantity, fraccion: safeQuantity % 1 } : row));
    }
    function saveDraft() {
    ...
    async function save() {
        ...
        const payload = { cliente: client, items: cart.map(i => ({ codigo: i.codigo, cantidad: i.unidadesSueltas, cantidadPresentacion: i.cantidad, factorPresentacion: i.factor, cantidadUnidadesBase: i.unidadesSueltas, fraccion: i.fraccion || 0, nombrePresentacion: i.nombrePresentacion || i.unidad || "Unidad", precioPresentacion: i.precioVenta, precioAplicado: i.cantidad * i.precioVenta, subtotal: i.cantidad * i.precioVenta })), observaciones: obs, solicitudId: crypto.randomUUID() };
...
function Collections({ rows, clients, loading, load, call, notify }: {
    ...
    function open(o: Order) { const hasPayment = Number(o.totalCobrado || 0) > 0; setSelected(o); setForm({ solicitudId: crypto.randomUUID(), resultado: "ENTREGA COMPLETA", entregado: true, fechaEntrega: o.fechaEntrega || today(), modoPago: hasPayment ? "MIXTO" : "CREDITO", cobrado: hasPayment, efectivo: Number(o.efectivo || 0), yape: Number(o.yape || 0), plin: Number(o.plin || 0), transferencia: Number(o.transferencia || 0), fechaPromesa: o.fechaPromesa || "", observacion: o.observacionCobro || "", ajuste: "" }); }
    ...
    async function save(e: FormEvent) {
        ...
        setSaving(true);
        try {
            const message = await call<string>("guardarCobranzaPedido", [{ ...selected, solicitudId: form.solicitudId, ventaId: selected.ventaId, cliente: selected.cliente, totalPedido: selected.total, estadoEntrega: form.entregado ? "ENTREGADO" : form.resultado, fechaEntrega: form.fechaEntrega, estadoPago: balance <= .01 ? "TOTAL" : paid > 0 ? "PARCIAL" : "PENDIENTE", efectivo, yape, plin, transferencia, pos: selected.pos || 0, otros: selected.otros || 0, fechaPromesa: balance > .01 && form.entregado ? (form.fechaPromesa || addDays(form.fechaEntrega, 3)) : "", medioPrometido: balance > .01 ? "POR CONFIRMAR" : "", observacion: form.observacion }]);
