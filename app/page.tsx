...
type JourneySummary = { fecha: string; jornada?: { id?: string; estado?: string; repartidor?: string }; pedidosAsignados: number; entregados: number; noEntregados: number; ventasEntregadas: number; totalCobrado: number; saldoPendiente: number; cobros: { total: number; efectivo: number; yape: number; plin: number; transferencia: number; otros: number }; saldos: { dentroPlazo: number; urgente: number }; gastos: { total: number; pendientes: number; aprobados: number; porCategoria: Record<string, number> }; efectivoEsperado: number };
...
function Orders({ orders, call, refresh, notify }: {
    ...
    const [orderTab, setOrderTab] = useState("POR COMPRAR");
    ...
    const states = ["POR COMPRAR", "LISTO PARA ENTREGA", "ENTREGADO", "OBSERVADO"];
    const stateOf = (o: Order) => String(o.estadoEntrega || "POR COMPRAR").toUpperCase();
    const counts = Object.fromEntries(states.map(state => [state, orders.filter(o => stateOf(o) === state).length]));
    const visible = orders.filter(o => stateOf(o) === orderTab && JSON.stringify(o).toLowerCase().includes(query.toLowerCase()));
    ...
    async function status(o: Order, value: string) {
        try {
            const observation = value === "OBSERVADO" ? window.prompt("Describe la incidencia u observación del pedido") : "";
            if (value === "OBSERVADO" && !observation?.trim()) return;
            let message = "";
            try {
                message = await call<string>("actualizarEstadoOperativoPedido", [{ ventaId: o.ventaId, cliente: o.cliente, totalPedido: o.total, estado: value, observacion: observation || "" }]);
            }
            catch (first) {
...
function Collections({ rows, clients, loading, load, call, notify }: {
    ...
    const loadJourney = useCallback(async () => {
        try {
            const summary = await measured("cargarJornada", () => call<JourneySummary>("obtenerResumenJornada", [today()]));
            setJourney(summary);
    ...
    const routeRows = rows.filter(o => String(o.estadoEntrega || "").toUpperCase() === "LISTO PARA ENTREGA");
    ...
    async function closeJourney() {
        ...
        try {
            ...
            if (result.ok) await loadJourney();
        }
