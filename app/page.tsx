...
function Orders({ orders, call, refresh, notify, onOrderUpdated }: {
    ...
    return <div className="orders-page orders-flow-v2">
        ...
        {detail && <div className="modal-bg order-dialog-bg" onMouseDown={e => { if(e.target===e.currentTarget) closeDetail(); }}><section className="order-detail order-dialog flow-order-detail"><button className="close" onClick={closeDetail}>×</button><small className="eyebrow">PEDIDO</small><h2>{detail.ventaId}</h2><p>{detail.cliente} · {detail.telefono || "Sin teléfono"}</p><div className={`emission-summary ${detail.codigoImpresion ? "emitted" : ""}`}><b>{detail.codigoImpresion ? "✓ Boleta emitida" : "Boleta no emitida"}</b>{detail.codigoImpresion && <small>{detail.codigoImpresion}</small>}</div><div className="order-progress"><span className="done">Pedido creado ✓</span><span className={stateOf(detail)!=="POR_COMPRAR" ? "done" : ""}>Preparación</span><span className={["LISTO_PARA_ENTREGA","ENTREGADO"].includes(stateOf(detail)) ? "done" : ""}>Listo</span><span className={detail.estadoEntrega==="EN_RUTA" || stateOf(detail)==="ENTREGADO" ? "done" : ""}>En ruta</span><span className={stateOf(detail)==="ENTREGADO" ? "done" : ""}>Entregado</span><span className={Number(detail.saldo || detail.total)<=.01 ? "done" : ""}>Cobrado</span></div>{stateOf(detail)==="POR_COMPRAR" && <section className="preparation-control"><header><h3>Preparación del pedido</h3><small>{autoSaveState}</small></header>{preparation.map((line,index) => { const requested=Number(line.cantidadPedido); return <article key={line.codigo}><span><b>{line.producto}</b><small>{line.codigo} · {line.presentacion || "Unidad"} x{line.factorPresentacion || 1}</small></span><label>Preparado<input type="number" min="0" max={requested} step=".01" value={line.cantidadPreparada || ""} onChange={e => updatePreparation(index,Number(e.target.value))}/></label><span className="picking-requested"><small>SOLICITADO</small><b>{requested} {line.presentacion || "Unidad"}</b></span><span className="picking-price"><small>PRECIO</small><b>{money(line.precio)}</b></span><strong className={`picking-${line.estadoLinea.toLowerCase()}`}>{line.estadoLinea === "PREPARADO" ? "✓ PREPARADO" : "⚠ PENDIENTE"}</strong></article>})}{allPrepared ? <p className="all-prepared">✓ TODOS LOS MATERIALES PREPARADOS</p> : <p className="preparation-warning">Faltan materiales por preparar.</p>}<div className="preparation-total"><span>TOTAL</span><b>{money(preparation.reduce((sum,line) => sum + Number(line.subtotal || 0),0))}</b></div><button className="primary" disabled={savingPreparation || !allPrepared} onClick={() => savePreparation(true)}>LISTO PARA ENTREGAR</button></section>}{stateOf(detail)==="LISTO_PARA_ENTREGA" && <button className="primary assign-journey-button" onClick={() => setAssignmentOpen(true)}>Asignar y enviar a ruta</button>}<h3>Materiales asociados</h3><div className="detail-items">{detail.items.map(item => <div className="detail-row" key={item.codigo}><span><b>{item.nombre}</b><small>{item.codigo}</small></span><b>{item.cantidad}</b><strong>{money(item.subtotal)}</strong></div>)}</div><div className="detail-total"><span>Total</span><strong>{money(detail.total)}</strong></div><div className="order-actions"><button onClick={() => startEdit(detail)}>Editar pedido</button><button disabled={printing} onClick={() => printOrder(detail)}>{detail.codigoImpresion ? "Reimprimir boleta" : "Emitir e imprimir"}</button></div></section></div>}
