...
function guardarCobranzaPedido(p){
  ...
  if(saldo>0.01&&entrega==='ENTREGADO'&&!p.fechaPromesa)return 'Indique la fecha prometida de pago para el saldo pendiente.';
...
function registrarGastoOperacion(p){requerirPerfil(['MASTER','PREVENTA']);asegurarCobranza_();p=p||{};if(!(Number(p.importe)>0))return 'Ingrese un importe válido.';ss().getSheetByName(HOJA_GASTOS_RUTA).appendRow(['GOP-'+Utilities.getUuid().slice(0,8).toUpperCase(),crearFecha(p.fecha||hoyClave_()),usuarioSistema_(),p.ruta||'',p.unidad||'',p.partida||'OTROS',p.descripcion||'',Number(p.importe)||0,String(p.canal||'EFECTIVO').toUpperCase(),p.comprobanteUrl||'',p.observacion||'','PENDIENTE',new Date()]);return 'Gasto operativo registrado correctamente. Pendiente de rendición.';}
