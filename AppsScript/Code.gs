...
function obtenerPreparacionPedido(ventaId){requerirPerfil(['MASTER','PREVENTA']);asegurarCobranza_();const venta=ventaPorId_(ventaId);if(!venta)return {ventaId,lineas:[],mensaje:'Pedido no encontrado.'};const existentes=preparacionMap_()[ventaId]||[],stock=obtenerStockMap_(),reservas=reservasStock_();const lineas=venta.items.map(it=>{const codigo=normalizarCodigo(it.codigo),actual=existentes.find(x=>x.codigo===codigo),base=Number(it.cantidadUnidadesBase!=null?it.cantidadUnidadesBase:(Number(it.cantidad)||0)*(Number(it.factorPresentacion)||1))||0,fisico=Number(stock[codigo])||0,reservado=Math.max(0,Number(reservas[codigo])||0);return Object.assign({ventaId,codigo,producto:String(it.nombre||codigo),cantidadPedido:Number(it.cantidad)||0,presentacion:String(it.nombrePresentacion||''),unidadesBase:base,cantidadPreparada:0,estadoLinea:'PENDIENTE',cantidadFaltante:0,motivo:'',observacion:'',estadoStock:'SIN RESERVA',stockFisico:fisico,stockReservado:reservado,stockDisponible:Math.max(0,fisico-reservado)},actual||{});});return {ventaId,cliente:venta.cliente,total:venta.total,lineas};}
...
function finalizarStockEntrega_(ventaId,cliente){const mov=ss().getSheetByName(HOJA_MOVIMIENTOS),d=mov.getDataRange().getValues();for(let i=1;i<d.length;i++)if(String(d[i][8])===String(ventaId))return false;const venta=ventaPorId_(ventaId);if(!venta)return false;const stock=obtenerStockMap_(),ahora=new Date(),vals=venta.items.map(it=>{const codigo=normalizarCodigo(it.codigo),base=Number(it.cantidadUnidadesBase!=null?it.cantidadUnidadesBase:(Number(it.cantidad)||0)*(Number(it.factorPresentacion)||1))||0,nuevo=Math.max(0,(Number(stock[codigo])||0)-base);return [codigo,ahora,'SALIDA',base,usuarioSistema_(),ahora,'SALIDA POR ENTREGA · Pedido '+ventaId,nuevo,ventaId,cliente||venta.cliente];});if(vals.length)mov.getRange(mov.getLastRow()+1,1,vals.length,10).setValues(vals);const prep=preparacionMap_()[ventaId]||[],ps=ss().getSheetByName(HOJA_PREPARACION);prep.forEach(x=>ps.getRange(x.fila,11).setValue('ENTREGADO'));return true;}
