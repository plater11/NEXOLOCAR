...
function registrarVenta(venta){
  ...
  try{
    ...
    const reservadoMap=typeof reservasStock_==='function'?reservasStock_():{};
    ...
    const items=venta.items.map(it=>{
      ...
      const st=Math.max(0,Math.round(((stockMap[c]||0)-(reservadoMap[c]||0))*100)/100);
      ...
      return {codigo:c,productoId:String(it.productoId||c),presentacionId:String(it.presentacionId||''),nombre:map[c].nombre,cantidad:cantidadPresentacion,cantidadEntera:Number(it.cantidadEntera!=null?it.cantidadEntera:Math.floor(cantidadPresentacion))||0,cantidadPresentacion:cantidadPresentacion,cantidadPresentacionTotal:cantidadPresentacion,factorPresentacion:factor,cantidadUnidadesBase:cantidadUnidadesBase,fraccion:Number(it.fraccion)||0,nombrePresentacion:String(it.nombrePresentacion||map[c].nombrePresentacion||map[c].unidad||'Unidad'),precioVenta:precioPresentacion,precioPresentacion:precioPresentacion,precioFraccionado:num(it.precioFraccionado),precioAplicado:subtotal,subtotal:subtotal,precioNormal:precioNormal,promocionAplicada:promocionAplicada?'SI':'NO',cantidadPromo:num(map[c].cantidadPromo),descripcionPromo:promocionAplicada?String(map[c].descripcionPromo||''):'',descuentoTotal:Math.round(descuentoUnitario*cantidadPresentacion*100)/100,stockNuevo:Math.max(0,stockMap[c])};
    });
    ...
    // El stock físico ya no sale en Preventa: se reserva al preparar y se descuenta al entregar.
    if(typeof asegurarCobranza_==='function'){asegurarCobranza_();const csh=ss().getSheetByName(HOJA_COBRANZA);csh.appendRow(['COB-'+Utilities.getUuid().slice(0,8).toUpperCase(),ventaId,fechaServidor,cliente,totalRedondeado,'POR COMPRAR','PENDIENTE',0,0,0,0,0,0,0,totalRedondeado,'','','',usuarioSistema_(),'PENDIENTE',fechaServidor]);registrarActividad_('NUEVO PEDIDO',ventaId,cliente,'Pedido generado desde Preventa',totalRedondeado);}
...
const HOJA_JORNADAS='Operacion_Jornadas';
const HOJA_ESTADOS_PEDIDO='Operacion_Estados_Historial';
const HOJA_PREPARACION='Operacion_Preparacion';
const HOJA_ACTIVIDAD='Operacion_Actividad';
function asegurarCobranza_(){
  ...
  const defs=[
    ...
    [HOJA_GASTOS_RUTA,['ID','Fecha','Responsable','Ruta','Unidad','Partida','Descripción','Importe','Canal','Comprobante URL','Observación','Estado Rendición','Timestamp','Estado Aprobación','Observación Administración','Resuelto Por','Fecha Resolución','Origen Dinero','Proveedor','Jornada ID']],
    ...
    [HOJA_JORNADAS,['Jornada ID','Fecha','Repartidor','Vehículo','Ruta','Estado','Pedidos Asignados','Entregados','No Entregados','Ventas Entregadas','Total Cobrado','Saldo Pendiente','Efectivo Esperado','Efectivo Entregado','Diferencia','Gastos Registrados','Gastos Pendientes Aprobación','Observación','Usuario Cierre','Timestamp']],
    [HOJA_ESTADOS_PEDIDO,['Fecha','Venta ID','Estado Anterior','Estado Nuevo','Usuario','Observación','Timestamp']],
    [HOJA_PREPARACION,['Venta ID','Código','Producto','Cantidad Pedido','Unidades Base','Cantidad Preparada','Estado Línea','Cantidad Faltante','Motivo','Observación','Estado Stock','Jornada ID','Repartidor','Vehículo','Ruta','Orden Visita','Usuario','Timestamp']],
    [HOJA_ACTIVIDAD,['ID','Fecha Hora','Tipo','Venta ID','Cliente','Descripción','Monto','Usuario']]
  ];
...
function registrarActividad_(tipo,ventaId,cliente,descripcion,monto){try{asegurarCobranza_();ss().getSheetByName(HOJA_ACTIVIDAD).appendRow(['ACT-'+Utilities.getUuid().slice(0,8).toUpperCase(),new Date(),tipo||'ACTIVIDAD',ventaId||'',cliente||'',descripcion||'',Number(monto)||0,usuarioSistema_()]);}catch(e){}}
function obtenerActividadReciente(limite){requerirPerfil(['MASTER','PREVENTA']);asegurarCobranza_();const d=ss().getSheetByName(HOJA_ACTIVIDAD).getDataRange().getValues(),n=Math.min(50,Math.max(1,Number(limite)||15));return d.slice(1).map(r=>({id:r[0],fecha:fechaSerializable_(r[1]),fechaOrden:crearFecha(r[1]).getTime(),tipo:String(r[2]),ventaId:String(r[3]||''),cliente:String(r[4]||''),descripcion:String(r[5]||''),monto:Number(r[6])||0,usuario:String(r[7]||'')})).sort((a,b)=>b.fechaOrden-a.fechaOrden).slice(0,n);}
function ventaPorId_(ventaId){const d=ss().getSheetByName(HOJA_VENTAS).getDataRange().getValues();for(let i=1;i<d.length;i++)if(String(d[i][0])===String(ventaId)){let items=[];try{items=JSON.parse(String(d[i][4]||'[]'));}catch(e){}return {ventaId:String(d[i][0]),fecha:d[i][1],cliente:String(d[i][2]),total:Number(d[i][3])||0,items};}return null;}
function preparacionMap_(){asegurarCobranza_();const d=ss().getSheetByName(HOJA_PREPARACION).getDataRange().getValues(),m={};for(let i=1;i<d.length;i++){const id=String(d[i][0]);if(!m[id])m[id]=[];m[id].push({fila:i+1,ventaId:id,codigo:String(d[i][1]),producto:String(d[i][2]),cantidadPedido:Number(d[i][3])||0,unidadesBase:Number(d[i][4])||0,cantidadPreparada:Number(d[i][5])||0,estadoLinea:String(d[i][6]||'PENDIENTE'),cantidadFaltante:Number(d[i][7])||0,motivo:String(d[i][8]||''),observacion:String(d[i][9]||''),estadoStock:String(d[i][10]||'SIN RESERVA'),jornadaId:String(d[i][11]||''),repartidor:String(d[i][12]||''),vehiculo:String(d[i][13]||''),ruta:String(d[i][14]||''),ordenVisita:Number(d[i][15])||0});}return m;}
function reservasStock_(){const m={},p=preparacionMap_();Object.values(p).flat().forEach(x=>{if(x.estadoStock==='RESERVADO'||x.estadoStock==='EN_RUTA')m[x.codigo]=(m[x.codigo]||0)+x.unidadesBase;});return m;}
function detalleReservasStock_(){const m={},p=preparacionMap_();Object.values(p).flat().forEach(x=>{if(!m[x.codigo])m[x.codigo]={reservado:0,enRuta:0};if(x.estadoStock==='RESERVADO')m[x.codigo].reservado+=x.unidadesBase;if(x.estadoStock==='EN_RUTA')m[x.codigo].enRuta+=x.unidadesBase;});return m;}
function estadosOperativosMap_(){asegurarCobranza_();const d=ss().getSheetByName(HOJA_ESTADOS_PEDIDO).getDataRange().getValues(),m={};for(let i=1;i<d.length;i++){const id=String(d[i][1]||'');if(id)m[id]=normalizarEstadoOperativo_(d[i][3]);}return m;}
function obtenerPreparacionPedido(ventaId){requerirPerfil(['MASTER','PREVENTA']);asegurarCobranza_();const venta=ventaPorId_(ventaId);if(!venta)return {ventaId,lineas:[],mensaje:'Pedido no encontrado.'};const existentes=preparacionMap_()[ventaId]||[],stock=obtenerStockMap_(),reservas=detalleReservasStock_();const lineas=venta.items.map(it=>{const codigo=normalizarCodigo(it.codigo),actual=existentes.find(x=>x.codigo===codigo),base=Number(it.cantidadUnidadesBase!=null?it.cantidadUnidadesBase:(Number(it.cantidad)||0)*(Number(it.factorPresentacion)||1))||0,fisico=Number(stock[codigo])||0,detalle=reservas[codigo]||{reservado:0,enRuta:0},reservado=Math.max(0,Number(detalle.reservado)||0),enRuta=Math.max(0,Number(detalle.enRuta)||0);return Object.assign({ventaId,codigo,producto:String(it.nombre||codigo),cantidadPedido:Number(it.cantidad)||0,presentacion:String(it.nombrePresentacion||''),unidadesBase:base,cantidadPreparada:0,estadoLinea:'PENDIENTE',cantidadFaltante:0,motivo:'',observacion:'',estadoStock:'SIN RESERVA',stockFisico:fisico,stockReservado:reservado,stockEnRuta:enRuta,stockDisponible:Math.max(0,fisico-reservado-enRuta)},actual||{});});return {ventaId,cliente:venta.cliente,total:venta.total,lineas};}
function guardarPreparacionPedido(p){requerirPerfil(['MASTER','PREVENTA']);asegurarCobranza_();p=p||{};const venta=ventaPorId_(p.ventaId),lineas=Array.isArray(p.lineas)?p.lineas:[];if(!venta||!lineas.length)return {ok:false,mensaje:'Preparación inválida.'};if(p.marcarListo&&lineas.some(x=>String(x.estadoLinea)!=='PREPARADO'))return {ok:false,mensaje:'Todas las líneas deben estar preparadas y sin faltantes.'};const sh=ss().getSheetByName(HOJA_PREPARACION),d=sh.getDataRange().getValues(),existentes=preparacionMap_()[p.ventaId]||[],reservas=reservasStock_(),stock=obtenerStockMap_();existentes.forEach(x=>{if(x.estadoStock==='RESERVADO'||x.estadoStock==='EN_RUTA')reservas[x.codigo]=Math.max(0,(Number(reservas[x.codigo])||0)-x.unidadesBase);});for(const x of lineas){const base=Number(x.unidadesBase)||0;if(p.marcarListo&&Math.max(0,(Number(stock[x.codigo])||0)-(Number(reservas[x.codigo])||0))<base)return {ok:false,mensaje:'Stock disponible insuficiente para '+x.producto+'.'};}for(let i=d.length-1;i>=1;i--)if(String(d[i][0])===String(p.ventaId))sh.deleteRow(i+1);const vals=lineas.map(x=>[p.ventaId,x.codigo,x.producto,Number(x.cantidadPedido)||0,Number(x.unidadesBase)||0,Number(x.cantidadPreparada)||0,String(x.estadoLinea||'PENDIENTE'),Number(x.cantidadFaltante)||0,x.motivo||'',x.observacion||'',p.marcarListo?'RESERVADO':String(x.estadoStock||'SIN RESERVA'),'','','','',0,usuarioSistema_(),new Date()]);sh.getRange(sh.getLastRow()+1,1,vals.length,vals[0].length).setValues(vals);if(p.marcarListo){actualizarEstadoOperativoPedido({ventaId:p.ventaId,cliente:venta.cliente,totalPedido:venta.total,estado:'LISTO PARA ENTREGA',observacion:'Preparación completada'});registrarActividad_('PEDIDO LISTO',p.ventaId,venta.cliente,'Preparación completada y stock reservado',venta.total);}return {ok:true,mensaje:p.marcarListo?'Pedido listo para entrega y stock reservado.':'Preparación guardada.'};}
function asignarPedidoJornada(p){requerirPerfil(['MASTER','PREVENTA']);asegurarCobranza_();p=p||{};const venta=ventaPorId_(p.ventaId),prep=preparacionMap_()[p.ventaId]||[];if(!venta||!prep.length||prep.some(x=>x.estadoStock!=='RESERVADO'))return {ok:false,mensaje:'El pedido debe estar listo y reservado antes de asignarlo.'};const sh=ss().getSheetByName(HOJA_PREPARACION);prep.forEach(x=>sh.getRange(x.fila,11,1,8).setValues([['EN_RUTA',p.jornadaId||('JOR-'+String(p.fecha||hoyClave_())),p.repartidor||'',p.vehiculo||'',p.ruta||'',Number(p.ordenVisita)||0,usuarioSistema_(),new Date()]]));const csh=ss().getSheetByName(HOJA_COBRANZA),cm=cobranzaMap_(),c=cm[p.ventaId]||{},row=[c.id||('COB-'+Utilities.getUuid().slice(0,8).toUpperCase()),p.ventaId,crearFecha(p.fecha||hoyClave_()),venta.cliente,venta.total,'EN_RUTA',c.estadoCobro||'PENDIENTE',c.efectivo||0,c.yape||0,c.plin||0,c.transferencia||0,c.pos||0,c.otros||0,c.totalCobrado||0,Math.max(0,venta.total-(c.totalCobrado||0)),c.fechaPromesa?crearFecha(c.fechaPromesa):'',c.medioPrometido||'',p.observacion||'',p.repartidor||usuarioSistema_(),'PENDIENTE',new Date()];if(c.fila)csh.getRange(c.fila,1,1,row.length).setValues([row]);else csh.appendRow(row);registrarActividad_('ASIGNADO A RUTA',p.ventaId,venta.cliente,[p.repartidor,p.vehiculo,p.ruta].filter(Boolean).join(' · '),venta.total);return {ok:true,mensaje:'Pedido asignado a jornada y stock movido a ruta.'};}
function finalizarStockEntrega_(ventaId,cliente,itemsEntregados){const mov=ss().getSheetByName(HOJA_MOVIMIENTOS),d=mov.getDataRange().getValues();for(let i=1;i<d.length;i++)if(String(d[i][8])===String(ventaId))return false;const venta=ventaPorId_(ventaId);if(!venta)return false;const cantidades={};(Array.isArray(itemsEntregados)?itemsEntregados:[]).forEach(x=>cantidades[normalizarCodigo(x.codigo)]=Math.max(0,Number(x.cantidadEntregada)||0));const parcial=Object.keys(cantidades).length>0,stock=obtenerStockMap_(),ahora=new Date(),vals=[];venta.items.forEach(it=>{const codigo=normalizarCodigo(it.codigo),pedido=Math.max(0,Number(it.cantidad)||0),entregado=parcial?Math.min(pedido,Number(cantidades[codigo])||0):pedido,totalBase=Number(it.cantidadUnidadesBase!=null?it.cantidadUnidadesBase:pedido*(Number(it.factorPresentacion)||1))||0,base=pedido>0?totalBase*(entregado/pedido):0;if(base<=0)return;const nuevo=Math.max(0,(Number(stock[codigo])||0)-base);vals.push([codigo,ahora,'SALIDA',base,usuarioSistema_(),ahora,(parcial?'SALIDA PARCIAL POR ENTREGA':'SALIDA POR ENTREGA')+' · Pedido '+ventaId,nuevo,ventaId,cliente||venta.cliente]);});if(vals.length)mov.getRange(mov.getLastRow()+1,1,vals.length,10).setValues(vals);const prep=preparacionMap_()[ventaId]||[],ps=ss().getSheetByName(HOJA_PREPARACION),salidas={};vals.forEach(v=>salidas[v[0]]=(salidas[v[0]]||0)+Number(v[3]||0));prep.forEach(x=>{if(!parcial){ps.getRange(x.fila,11).setValue('ENTREGADO');return;}const restante=Math.max(0,x.unidadesBase-(salidas[x.codigo]||0));ps.getRange(x.fila,5).setValue(restante);ps.getRange(x.fila,11).setValue(restante>0?'RESERVADO':'ENTREGADO');});return true;}
function actualizarReservaEntrega_(ventaId,estado){const prep=preparacionMap_()[ventaId]||[],sh=ss().getSheetByName(HOJA_PREPARACION);prep.forEach(x=>sh.getRange(x.fila,11).setValue(estado));}
...
function actualizarEstadoOperativoPedido(p){
  ...
  const ventaId=String(p.ventaId||'').trim(), permitidos=['POR COMPRAR','LISTO PARA ENTREGA','ENTREGADO','OBSERVADO'];
  ...
  if(estado==='OBSERVADO'&&!String(p.observacion||'').trim())return 'Indique la observación o incidencia del pedido.';
  ...
  ss().getSheetByName(HOJA_ESTADOS_PEDIDO).appendRow([new Date(),ventaId,String(actual.estadoEntrega||'POR COMPRAR'),estado,usuarioSistema_(),String(p.observacion||''),new Date()]);
  registrarActividad_(estado==='OBSERVADO'?'PEDIDO OBSERVADO':'ESTADO DE PEDIDO',ventaId,p.cliente||actual.cliente,'Estado: '+estado,Number(total)||0);
  SpreadsheetApp.flush();
...
function guardarCobranzaPedido(p){
  ...
  if(map[p.ventaId])sh.getRange(map[p.ventaId].fila,1,1,row.length).setValues([row]);else sh.appendRow(row);actualizarCxC_(Object.assign({},p,{totalCobrado:cob,estadoCobro:estado}));if(entrega==='ENTREGADO'){finalizarStockEntrega_(p.ventaId,p.cliente);actualizarEstadoOperativoPedido({ventaId:p.ventaId,cliente:p.cliente,totalPedido:total,estado:'ENTREGADO',observacion:p.observacion||'Entrega completada'});registrarActividad_(cob>0?'ENTREGA Y COBRO':'ENTREGA REALIZADA',p.ventaId,p.cliente,'Saldo '+saldo.toFixed(2),cob);}else if(entrega==='ENTREGA PARCIAL'){finalizarStockEntrega_(p.ventaId,p.cliente,p.itemsEntregados);actualizarEstadoOperativoPedido({ventaId:p.ventaId,cliente:p.cliente,totalPedido:total,estado:'OBSERVADO',observacion:p.observacion||'Entrega parcial'});registrarActividad_('ENTREGA PARCIAL',p.ventaId,p.cliente,String(p.observacion||''),cob);}else{if(entrega==='RECHAZADO'||entrega==='DIRECCIÓN INCORRECTA')actualizarReservaEntrega_(p.ventaId,'DISPONIBLE');else actualizarReservaEntrega_(p.ventaId,'EN_RUTA');actualizarEstadoOperativoPedido({ventaId:p.ventaId,cliente:p.cliente,totalPedido:total,estado:'OBSERVADO',observacion:entrega+' · '+String(p.observacion||'')});registrarActividad_('ENTREGA OBSERVADA',p.ventaId,p.cliente,entrega+' · '+String(p.observacion||''),0);}if(cacheKey)cache.put(cacheKey,String(p.ventaId),21600);return 'Cobranza registrada correctamente. Pendiente de rendición administrativa.';
}
function obtenerHistorialEstadosPedido(ventaId){requerirPerfil(['MASTER','PREVENTA']);asegurarCobranza_();const d=ss().getSheetByName(HOJA_ESTADOS_PEDIDO).getDataRange().getValues();return d.slice(1).filter(r=>String(r[1])===String(ventaId)).map(r=>({fecha:fechaSerializable_(r[0]),ventaId:r[1],anterior:r[2],nuevo:r[3],usuario:r[4],observacion:r[5]})).reverse();}
...
function registrarGastoOperacion(p){
  ...
  const lock=LockService.getScriptLock();try{lock.waitLock(8000);if(cacheKey&&cache.get(cacheKey))return 'Gasto ya registrado. Se evitó duplicarlo.';const id='GOP-'+Utilities.getUuid().slice(0,8).toUpperCase();ss().getSheetByName(HOJA_GASTOS_RUTA).appendRow([id,crearFecha(p.fecha||hoyClave_()),usuarioSistema_(),p.ruta||'',p.unidad||'',p.partida||'OTROS',p.descripcion||'',Number(p.importe)||0,String(p.canal||'EFECTIVO').toUpperCase(),p.comprobanteUrl||'',p.observacion||'','PENDIENTE',new Date(),'PENDIENTE','','','',p.origenDinero||'FONDO DE RUTA',p.proveedor||'',p.jornadaId||('JOR-'+String(p.fecha||hoyClave_()))]);registrarActividad_('GASTO DE RUTA','',usuarioSistema_(),String(p.partida||'OTROS')+' · '+String(p.descripcion||''),Number(p.importe)||0);if(cacheKey)cache.put(cacheKey,id,21600);return 'Gasto registrado. Pendiente de aprobación financiera.';}finally{try{lock.releaseLock();}catch(err){}}
}
function obtenerGastosOperacion(fecha){requerirPerfil(['MASTER','PREVENTA']);asegurarCobranza_();if(fecha==null)fecha=hoyClave_();const todos=fecha==='*',d=ss().getSheetByName(HOJA_GASTOS_RUTA).getDataRange().getValues(),o=[];for(let i=1;i<d.length;i++)if(todos||fechaClave_(d[i][1])===fecha)o.push({id:d[i][0],fecha:fechaClave_(d[i][1]),responsable:d[i][2],ruta:d[i][3],unidad:d[i][4],partida:d[i][5],descripcion:d[i][6],importe:Number(d[i][7])||0,canal:d[i][8],comprobanteUrl:d[i][9],observacion:d[i][10],estadoRendicion:d[i][11],estadoAprobacion:String(d[i][13]||'PENDIENTE'),observacionAdministracion:String(d[i][14]||''),resueltoPor:String(d[i][15]||''),fechaResolucion:fechaSerializable_(d[i][16]),origenDinero:String(d[i][17]||''),proveedor:String(d[i][18]||''),jornadaId:String(d[i][19]||'')});return o;}
function obtenerGastosPendientes(){requerirPerfil(['MASTER']);asegurarCobranza_();return obtenerGastosOperacion('*').filter(g=>g.estadoAprobacion==='PENDIENTE'||g.estadoAprobacion==='OBSERVADO');}
function resolverGastoOperacion(p){requerirPerfil(['MASTER']);asegurarCobranza_();p=p||{};const estado=String(p.estado||'').toUpperCase();if(['APROBADO','OBSERVADO','RECHAZADO'].indexOf(estado)<0)return 'Estado de aprobación inválido.';if((estado==='OBSERVADO'||estado==='RECHAZADO')&&!String(p.observacion||'').trim())return 'Indique el motivo.';const sh=ss().getSheetByName(HOJA_GASTOS_RUTA),d=sh.getDataRange().getValues();for(let i=1;i<d.length;i++)if(String(d[i][0])===String(p.id||p.solicitudId)){sh.getRange(i+1,14,1,4).setValues([[estado,String(p.observacion||''),usuarioSistema_(),new Date()]]);if(estado==='APROBADO')volcarDiaAContabilidad_(fechaClave_(d[i][1]));registrarActividad_('GASTO '+estado,'',String(d[i][2]||''),String(d[i][5]||'')+' · '+String(p.observacion||''),Number(d[i][7])||0);return 'Gasto '+estado.toLowerCase()+' correctamente.';}return 'Gasto no encontrado.';}
...
function obtenerResumenJornada(fecha){requerirPerfil(['MASTER','PREVENTA']);asegurarCobranza_();fecha=fecha||hoyClave_();const cm=cobranzaMap_(),gastos=obtenerGastosOperacion(fecha),pedidos=Object.values(cm).filter(c=>c.fecha===fecha),entregados=pedidos.filter(c=>String(c.estadoEntrega).toUpperCase()==='ENTREGADO'),noEntregados=pedidos.filter(c=>String(c.estadoEntrega).toUpperCase()!=='ENTREGADO'),sum=(arr,k)=>arr.reduce((a,x)=>a+(Number(x[k])||0),0),efectivo=sum(entregados,'efectivo'),yape=sum(entregados,'yape'),plin=sum(entregados,'plin'),transferencia=sum(entregados,'transferencia'),otros=entregados.reduce((a,x)=>a+(Number(x.pos)||0)+(Number(x.otros)||0),0),totalCobrado=efectivo+yape+plin+transferencia+otros,saldoPendiente=sum(entregados,'saldo'),ventasEntregadas=sum(entregados,'total'),gastosTotal=sum(gastos,'importe'),gastosPendientes=sum(gastos.filter(g=>g.estadoAprobacion==='PENDIENTE'||g.estadoAprobacion==='OBSERVADO'),'importe'),gastosAprobados=sum(gastos.filter(g=>g.estadoAprobacion==='APROBADO'),'importe'),js=ss().getSheetByName(HOJA_JORNADAS),jd=js.getDataRange().getValues();let jornada=null;for(let i=1;i<jd.length;i++)if(fechaClave_(jd[i][1])===fecha)jornada={id:jd[i][0],estado:String(jd[i][5]||'ABIERTA'),repartidor:String(jd[i][2]||''),vehiculo:String(jd[i][3]||''),ruta:String(jd[i][4]||''),efectivoEntregado:Number(jd[i][13])||0,diferencia:Number(jd[i][14])||0,observacion:String(jd[i][17]||'')};return {fecha,jornada:jornada||{id:'JOR-'+fecha,estado:'ABIERTA',repartidor:usuarioSistema_(),vehiculo:'',ruta:''},pedidosAsignados:pedidos.length,entregados:entregados.length,noEntregados:noEntregados.length,ventasEntregadas,totalCobrado,saldoPendiente,cobros:{efectivo,yape,plin,transferencia,otros,total:totalCobrado},saldos:{dentroPlazo:entregados.filter(x=>x.saldo>0&&diasEntre_(x.fecha,hoyClave_())<=3).reduce((a,x)=>a+x.saldo,0),urgente:entregados.filter(x=>x.saldo>0&&diasEntre_(x.fecha,hoyClave_())>3).reduce((a,x)=>a+x.saldo,0)},noEntregadosDetalle:{clienteAusente:noEntregados.filter(x=>/AUSENTE/i.test(x.observacion)).length,reprogramados:noEntregados.filter(x=>/REPROGRAM/i.test(x.observacion)).length,rechazados:noEntregados.filter(x=>/RECHAZ/i.test(x.observacion)).length,pendientes:noEntregados.filter(x=>!/AUSENTE|REPROGRAM|RECHAZ/i.test(x.observacion)).length},gastos:{total:gastosTotal,pendientes:gastosPendientes,aprobados:gastosAprobados,porCategoria:gastos.reduce((m,g)=>{m[g.partida]=(m[g.partida]||0)+g.importe;return m;},{})},efectivoEsperado:Math.max(0,efectivo-gastos.filter(g=>String(g.canal).toUpperCase()==='EFECTIVO').reduce((a,g)=>a+g.importe,0))};}
function diasEntre_(a,b){const x=crearFecha(a),y=crearFecha(b);return Math.max(0,Math.floor((y-x)/86400000));}
function cerrarJornada(p){requerirPerfil(['MASTER','PREVENTA']);asegurarCobranza_();p=p||{};const fecha=String(p.fecha||hoyClave_()),r=obtenerResumenJornada(fecha),entregado=Number(p.efectivoEntregado)||0,dif=Math.round((entregado-r.efectivoEsperado)*100)/100;if(r.noEntregados>0&&!String(p.observacionNoEntregados||'').trim())return {ok:false,mensaje:'Identifique los pedidos no entregados antes de cerrar.'};if(Math.abs(dif)>.01&&!String(p.observacion||'').trim())return {ok:false,mensaje:'La diferencia de efectivo requiere una observación.'};const sh=ss().getSheetByName(HOJA_JORNADAS),d=sh.getDataRange().getValues(),row=[r.jornada.id||('JOR-'+fecha),crearFecha(fecha),p.repartidor||r.jornada.repartidor||usuarioSistema_(),p.vehiculo||r.jornada.vehiculo||'',p.ruta||r.jornada.ruta||'','CERRADA',r.pedidosAsignados,r.entregados,r.noEntregados,r.ventasEntregadas,r.totalCobrado,r.saldoPendiente,r.efectivoEsperado,entregado,dif,r.gastos.total,r.gastos.pendientes,String(p.observacion||p.observacionNoEntregados||''),usuarioSistema_(),new Date()];let fila=0;for(let i=1;i<d.length;i++)if(fechaClave_(d[i][1])===fecha)fila=i+1;if(fila)sh.getRange(fila,1,1,row.length).setValues([row]);else sh.appendRow(row);return {ok:true,mensaje:'Jornada cerrada correctamente.',resumen:r,diferencia:dif};}
...
function volcarDiaAContabilidad_(fecha){
  ...
  obtenerGastosOperacion(fecha).filter(g=>g.estadoAprobacion==='APROBADO').forEach(g=>add('OPERACIÓN GASTO',String(g.partida||'OTROS').toUpperCase(),g.descripcion||g.partida,g.importe,String(g.canal||'EFECTIVO').toUpperCase()));if(vals.length)sh.getRange(sh.getLastRow()+1,1,vals.length,vals[0].length).setValues(vals);
}
