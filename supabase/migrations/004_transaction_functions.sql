...
declare v_total numeric;v_pagado numeric;v_monto numeric;v_pago uuid;v_cliente uuid;begin
  ...
  update pedidos set estado_cobranza=(case when v_pagado+v_monto>=v_total-.01 then 'COBRADO' else 'PENDIENTE_DE_COBRO' end)::public.estado_cobranza,version=version+1,updated_at=now() where id=p_pedido_id; perform registrar_evento('PAGO_REGISTRADO','PEDIDO',p_pedido_id,'Pago registrado',v_monto); return jsonb_build_object('success',true,'pago_id',v_pago,'monto',v_monto);
end $$;
