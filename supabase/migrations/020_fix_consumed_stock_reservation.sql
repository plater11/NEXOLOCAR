-- A consumed reservation keeps its original audited quantity. The previous
-- implementation attempted to persist zero and violated cantidad > 0.
...
begin
  ...
  if v_idempotency_key is not null then
    ...
    if found then
      return jsonb_build_object('success',true,'entrega_id',v_entrega,'duplicate',true);
    end if;
  end if;
  ...
  if v_estado in ('ENTREGA_COMPLETA','ENTREGA_PARCIAL') then
    for v_item in
      select (x->>'producto_id')::uuid producto_id,(x->>'cantidad_pedida')::numeric pedida,(x->>'cantidad_entregada')::numeric entregada
      from jsonb_array_elements(p_payload->'items') x
    loop
      insert into entrega_detalle(entrega_id,producto_id,cantidad_pedida,cantidad_entregada)
      values(v_entrega,v_item.producto_id,v_item.pedida,v_item.entregada);

      select * into v_stock from reservas_stock
      where pedido_id=p_pedido_id and producto_id=v_item.producto_id and estado='EN_RUTA'
      for update;
      if not found then raise exception 'Reserva en ruta no encontrada'; end if;

      update stock_actual
      set stock_fisico=stock_fisico-v_item.entregada,
          stock_en_ruta=stock_en_ruta-v_stock.cantidad,
          stock_reservado=stock_reservado+greatest(v_stock.cantidad-v_item.entregada,0),
          updated_at=now()
      where producto_id=v_item.producto_id and almacen_id=v_stock.almacen_id;
      ...
      select v_item.producto_id,v_stock.almacen_id,'SALIDA_ENTREGA',v_item.entregada,'ENTREGA',v_entrega,stock_fisico+v_item.entregada,stock_fisico,auth.uid(),v_key,'Salida definitiva por entrega'
      from stock_actual where producto_id=v_item.producto_id and almacen_id=v_stock.almacen_id;

      update reservas_stock
      set cantidad=case when cantidad-v_item.entregada>0 then cantidad-v_item.entregada else cantidad end,
          estado=case when cantidad-v_item.entregada>0 then 'RESERVADA' else 'CONSUMIDA' end,
          updated_at=now()
      where id=v_stock.id;
    end loop;

    update pedidos
    set estado_operativo=case when v_estado='ENTREGA_COMPLETA' then 'ENTREGADO' else 'OBSERVADO' end,
        estado_entrega=v_estado,
        estado_cobranza='PENDIENTE_DE_COBRO',
        version=version+1,
        updated_at=now()
    where id=p_pedido_id;
  elsif v_estado in ('RECHAZADO','DIRECCION_INCORRECTA') then
    update stock_actual s set stock_en_ruta=s.stock_en_ruta-r.cantidad,updated_at=now()
    from reservas_stock r
    where r.pedido_id=p_pedido_id and r.estado='EN_RUTA' and s.producto_id=r.producto_id and s.almacen_id=r.almacen_id;
